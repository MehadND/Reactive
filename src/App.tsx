...
function App() {
  ...
  const [showEditor, setShowEditor] =
    useState<StateContextType["showEditor"]>(false);
  const [sourceURLs, setSourceUrls] = useState<StateContextType["sourceURLs"]>(
    []
  );
  const [videoThumbnails, setVideoThumbnails] = useState<
    StateContextType["videoThumbnails"]
  >([]);
  const [currUrlIdx, setCurrUrlidx] =
    useState<StateContextType["currUrlIdx"]>(0);
  const [splitTimeStamps, setSplitTimeStamps] = useState<
    StateContextType["splitTimeStamps"]
  >([]);
  ...
  const removeVideo = (index: number) => {
    if (sourceURLs.length === 0) {
      ...
      const confirmationDialog = window.confirm(
      ...
      <div className={`${theme}-theme-bg page-container`}>
        ...
        <div
          className={`footer-container ${
            theme === "dark" ? "dark-footer" : "light-footer"
          }`}
        >
          <div
          ...
          <div
            ...
            onClick={() =>
              window.open("https://github.com/Prakshal-Jain/Reactive", "_blank")
            }
          >
            <FontAwesomeIcon
              icon={faGithub}
